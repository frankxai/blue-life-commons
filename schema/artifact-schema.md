...
- `gallery`: curated multi-angle / life-stage educational stills for species pages. Each item needs `public_media_url`, rights fields, `qa_status`, and preferably `life_stage` + `angle`. Real photos require `qa_status: approved`; generated items use `rights_status: concept-reconstruction` (or `generated-owned`) and `concept_only: true`.
- `video_links`: external educational videos (NOAA, sanctuaries, open education). Link-out cards only unless rehost rights are explicit. Include `title`, `url`, `provider`, `topic`, and a short `rights_note`.
